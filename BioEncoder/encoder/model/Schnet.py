...
class Encoder(torch.nn.Module):
    ...
    def __init__(self, args, node_level=False, graph_level=True):
        ...
        if args.encoder == 'schnet':
            self.encoder = SchNet(cutoff=self.cutoff, num_layers=self.num_layers, hidden_channels=self.hidden_channels,
                                  dropout_rate=self.dropout_rate, num_filters=self.num_filters,
                                  num_gaussians=self.num_gaussians)
        elif args.encoder == 'spherenet':
            self.encoder = SphereNet(cutoff=self.cutoff, num_layers=self.num_layers,
                                     hidden_channels=self.hidden_channels,  # out_channels=1,
                                     ...
                                     num_output_layers=self.num_output_layers, dropout_rate=self.dropout_rate,
            ...
            self.encoder = GCN(feat_dim=self.feat_dim, hidden_dim=self.hidden_channels, n_layers=self.num_layers,
                               pool=self.pool, bn=self.bn, act=self.act, bias=self.bias, xavier=self.xavier,
...
class SchNet(torch.nn.Module):
    ...
    def __init__(self, num_layers = 2, cutoff = 5.0, hidden_channels = 128, dropout_rate = 0.1, num_filters=128, num_gaussians=50):
        ...
        self.update_vs = torch.nn.ModuleList(
            [update_v(hidden_channels, num_filters, dropout_rate) for _ in range(num_layers)])
